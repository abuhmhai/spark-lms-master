...
public class BookController {
	...
	public String saveBook(@Valid @ModelAttribute("book") Book book, BindingResult bindingResult, final RedirectAttributes redirectAttributes) {
		if (bindingResult.hasErrors()) {
			return "/book/form"; // Trả về cùng một trang với thông báo lỗi
		}

		// Thay đổi so sánh với null từ int sang Integer
		if (book.getId() == null) { // Thêm mới sách
			if (bookService.getByTag(book.getTag()) != null) {
				bindingResult.rejectValue("tag", "tag", "Tag already exists");
				return "/book/form";
			} else {
				bookService.addNew(book);
				redirectAttributes.addFlashAttribute("successMsg", "'" + book.getTitle() + "' has been added as a new book.");
				return "redirect:/book/list"; // Chuyển hướng đến trang danh sách
			}
		} else { // Cập nhật sách
